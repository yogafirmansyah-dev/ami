        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (__props.stageBreakdown) {
              _push2(`<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-10" data-v-84b638d0${_scopeId}><!--[-->`);
              ssrRenderList(__props.stageBreakdown, (stat) => {
                _push2(`<div class="bg-white dark:bg-slate-900 backdrop-blur-3xl p-5 rounded-[2rem] border border-white/40 dark:border-white/5 shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-none transition-all hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 group relative overflow-hidden" data-v-84b638d0${_scopeId}><p class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] leading-none mb-3 group-hover:text-rose-500 transition-colors" data-v-84b638d0${_scopeId}>${ssrInterpolate(stat.label)}</p><div class="flex items-end justify-between relative z-10" data-v-84b638d0${_scopeId}><p class="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tighter italic" data-v-84b638d0${_scopeId}>${ssrInterpolate(stat.total)}</p><div class="w-1.5 h-1.5 rounded-full bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.5)] animate-pulse mb-1.5" data-v-84b638d0${_scopeId}></div></div></div>`);
              });
              _push2(`<!--]--></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div class="space-y-6" data-v-84b638d0${_scopeId}><div class="flex flex-col lg:flex-row justify-between items-center gap-6" data-v-84b638d0${_scopeId}><div class="flex items-stretch gap-3 w-full max-w-2xl" data-v-84b638d0${_scopeId}><div class="relative flex-1 group" data-v-84b638d0${_scopeId}><span class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none" data-v-84b638d0${_scopeId}>`);
            if (!isSearching.value) {
              _push2(`<svg class="h-4 w-4 text-slate-400 group-focus-within:text-rose-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-84b638d0${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" data-v-84b638d0${_scopeId}></path></svg>`);
            } else {
              _push2(`<div class="h-4 w-4 border-2 border-rose-500 border-t-transparent rounded-full animate-spin" data-v-84b638d0${_scopeId}></div>`);
            }
            _push2(`</span><input${ssrRenderAttr("value", search.value)} type="text" placeholder="Cari unit, auditor, atau standar..." class="w-full pl-11 pr-4 py-4 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder:text-slate-400/50 font-bold text-xs rounded-2xl border-none outline-none focus:outline-none ring-1 ring-slate-200 dark:ring-slate-800 dark:focus:ring-rose-500/50 focus:ring-4 focus:ring-rose-500/50 shadow-sm focus:shadow-md transition-[ring,background-color,box-shadow] duration-300 ease-out focus:bg-slate-50 dark:focus:bg-slate-800/80" data-v-84b638d0${_scopeId}></div><div class="flex items-center bg-white dark:bg-slate-900 rounded-2xl px-4 ring-1 ring-slate-200 dark:ring-slate-800 shadow-sm hover:ring-rose-500/50 dark:hover:ring-rose-500/50 transition-all duration-300" data-v-84b638d0${_scopeId}><span class="hidden sm:inline text-[9px] font-black uppercase text-slate-400 px-2 border-r border-slate-200 dark:border-slate-800 mr-2" data-v-84b638d0${_scopeId}>Tampilkan</span><select class="bg-transparent dark:bg-slate-900 border-none focus:ring-0 text-xs font-black text-slate-900 dark:text-white py-4 pr-8 cursor-pointer hover:text-rose-500 dark:hover:text-rose-500 transition-colors duration-300 outline-none" data-v-84b638d0${_scopeId}><option${ssrRenderAttr("value", 10)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(perPage.value) ? ssrLooseContain(perPage.value, 10) : ssrLooseEqual(perPage.value, 10)) ? " selected" : ""}${_scopeId}>10</option><option${ssrRenderAttr("value", 25)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(perPage.value) ? ssrLooseContain(perPage.value, 25) : ssrLooseEqual(perPage.value, 25)) ? " selected" : ""}${_scopeId}>25</option><option${ssrRenderAttr("value", 50)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(perPage.value) ? ssrLooseContain(perPage.value, 50) : ssrLooseEqual(perPage.value, 50)) ? " selected" : ""}${_scopeId}>50</option><option${ssrRenderAttr("value", 100)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(perPage.value) ? ssrLooseContain(perPage.value, 100) : ssrLooseEqual(perPage.value, 100)) ? " selected" : ""}${_scopeId}>100</option></select></div></div><button class="group w-full lg:w-auto inline-flex items-center justify-center px-8 py-3.5 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 dark:from-rose-600 dark:via-rose-500 dark:to-rose-600 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-slate-900/20 dark:shadow-rose-600/30 transition-all hover:shadow-2xl hover:shadow-slate-900/30 dark:hover:shadow-rose-600/40 hover:-translate-y-0.5 active:scale-95 border-t border-white/10" data-v-84b638d0${_scopeId}><span class="mr-2 text-rose-400 dark:text-white group-hover:rotate-90 transition-transform duration-300 text-sm leading-none" data-v-84b638d0${_scopeId}>+</span> Inisialisasi AMI Baru </button></div><div class="bg-white/60 dark:bg-slate-900 backdrop-blur-3xl rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm overflow-hidden" data-v-84b638d0${_scopeId}><div class="overflow-x-auto custom-scrollbar" data-v-84b638d0${_scopeId}><table class="w-full text-left border-collapse min-w-[1000px] lg:min-w-full" data-v-84b638d0${_scopeId}><thead data-v-84b638d0${_scopeId}><tr class="bg-slate-50/80 dark:bg-slate-800/20 text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-[0.2em] sticky top-0 z-20 border-b border-slate-100 dark:border-slate-800/50" data-v-84b638d0${_scopeId}><th class="p-6 md:p-8" data-v-84b638d0${_scopeId}>No</th><th class="p-6 md:p-8 pl-8" data-v-84b638d0${_scopeId}>Unit Kerja &amp; Kategori</th><th class="p-6 md:p-8" data-v-84b638d0${_scopeId}>Metadata AMI</th><th class="p-6 md:p-8" data-v-84b638d0${_scopeId}>Auditor</th><th class="p-6 md:p-8 text-center" data-v-84b638d0${_scopeId}>Current Progress</th><th class="p-6 md:p-8 pr-8 text-right" data-v-84b638d0${_scopeId}>Actions</th></tr></thead><tbody class="divide-y divide-slate-50 dark:divide-slate-800/20" data-v-84b638d0${_scopeId}><!--[-->`);
            ssrRenderList(__props.assignments.data, (item) => {
              _push2(`<tr class="group hover:bg-white/50 dark:hover:bg-white/[0.02] transition-colors duration-300" data-v-84b638d0${_scopeId}><td class="p-6 md:p-8" data-v-84b638d0${_scopeId}><span class="font-mono text-sm font-black text-rose-500 bg-rose-50 dark:bg-rose-500/10 px-2 py-1 rounded-md border border-rose-100 dark:border-rose-500/20" data-v-84b638d0${_scopeId}>${ssrInterpolate(__props.assignments.from + __props.assignments.data.indexOf(item))}</span></td><td class="p-6 md:p-8 pl-8" data-v-84b638d0${_scopeId}><div class="flex items-center gap-4" data-v-84b638d0${_scopeId}><div class="${ssrRenderClass([
              ...
              ])}" data-v-84b638d0${_scopeId}>${ssrInterpolate(item.assignable?.name?.charAt(0))}</div><div data-v-84b638d0${_scopeId}><div class="font-black text-slate-800 dark:text-slate-200 uppercase text-xs tracking-tight italic leading-none mb-1.5" data-v-84b638d0${_scopeId}>${ssrInterpolate(item.assignable?.name)}</div><div class="flex items-center gap-2" data-v-84b638d0${_scopeId}><span class="${ssrRenderClass([
              ...
              ])}" data-v-84b638d0${_scopeId}>${ssrInterpolate(item.assignable_type.includes("Prodi") ? "Program Studi" : "Fakultas")}</span></div></div></div></td><td class="p-6 md:p-8" data-v-84b638d0${_scopeId}><div class="text-[11px] font-bold text-slate-600 dark:text-slate-300 leading-snug" data-v-84b638d0${_scopeId}>${ssrInterpolate(item.standard?.name)}</div><div class="text-[9px] font-black text-slate-400 dark:text-slate-600 mt-1.5 uppercase tracking-tighter italic flex items-center gap-1" data-v-84b638d0${_scopeId}>`);
              _push2(ssrRenderComponent(_component_icon, {
                icon: "fa-regular fa-calendar",
                class: "text-[8px]"
              }, null, _parent2, _scopeId));
              _push2(` ${ssrInterpolate(item.period?.name)}</div></td><td class="p-6 md:p-8 text-slate-600 dark:text-slate-400 font-bold text-[10px]" data-v-84b638d0${_scopeId}>`);
              if (item.auditor) {
                _push2(`<div class="flex items-center gap-2.5" data-v-84b638d0${_scopeId}><div class="w-8 h-8 rounded-lg bg-slate-900 dark:bg-slate-800 text-white flex items-center justify-center font-black text-[9px] shadow-lg shadow-slate-900/10 uppercase italic border border-white/10" data-v-84b638d0${_scopeId}>${ssrInterpolate(item.auditor.name.substring(0, 2))}</div><span class="tracking-tight" data-v-84b638d0${_scopeId}>${ssrInterpolate(item.auditor.name)}</span></div>`);
              } else {
                _push2(`<span class="text-[9px] font-black text-slate-300 dark:text-slate-700 uppercase tracking-widest animate-pulse" data-v-84b638d0${_scopeId}>Belum Diplot</span>`);
              }
              _push2(`</td><td class="p-6 md:p-8" data-v-84b638d0${_scopeId}><div class="max-w-[140px] mx-auto space-y-2" data-v-84b638d0${_scopeId}><div class="flex justify-between items-center text-[8px] font-black uppercase" data-v-84b638d0${_scopeId}><span class="${ssrRenderClass(["px-2.5 py-1 rounded-md border border-transparent italic shadow-sm transition-all", getStageConfig(item.current_stage).color])}" data-v-84b638d0${_scopeId}>${ssrInterpolate(getStageConfig(item.current_stage).label)}</span><span class="text-slate-700 dark:text-slate-300 italic tracking-tighter font-black" data-v-84b638d0${_scopeId}>${ssrInterpolate(getProgress(item))}%</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 h-1 rounded-full overflow-hidden p-[1px]" data-v-84b638d0${_scopeId}><div class="bg-slate-900 dark:bg-rose-600 h-full rounded-full transition-all duration-1000 shadow-[0_0_8px_rgba(244,63,94,0.4)]" style="${ssrRenderStyle({ width: getProgress(item) + "%" })}" data-v-84b638d0${_scopeId}></div></div></div></td><td class="p-6 md:p-8 pr-8 text-right" data-v-84b638d0${_scopeId}><div class="flex justify-end items-center gap-2" data-v-84b638d0${_scopeId}>`);
              ...
              _push2(`<button class="w-8 h-8 flex items-center justify-center text-slate-300 dark:text-slate-700 hover:text-rose-600 dark:hover:text-rose-500 transition-all hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl active:scale-95" data-v-84b638d0${_scopeId}>`);
              _push2(ssrRenderComponent(_component_icon, {
            ...
            _push2(`<!--]--></tbody></table></div><div class="flex flex-col md:flex-row items-center justify-between gap-4 px-6 md:px-8 py-6 border-t border-slate-100 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-800/20 backdrop-blur-sm" data-v-84b638d0${_scopeId}><div class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic text-center md:text-left" data-v-84b638d0${_scopeId}> Data ${ssrInterpolate(__props.assignments.from)} - ${ssrInterpolate(__props.assignments.to)} dari total ${ssrInterpolate(__props.assignments.total)}</div><div class="flex flex-wrap justify-center gap-1.5" data-v-84b638d0${_scopeId}><!--[-->`);
            ...
            if (showModal.value) {
              _push2(`<div class="fixed inset-0 z-50 flex items-center justify-center p-6" data-v-84b638d0${_scopeId}><div class="fixed inset-0 bg-slate-900/80 dark:bg-slate-950/90 backdrop-blur-md transition-opacity" data-v-84b638d0${_scopeId}></div><div class="relative bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[4rem] shadow-2xl overflow-hidden border border-white/10 animate-in zoom-in duration-300" data-v-84b638d0${_scopeId}><div class="p-12 border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30" data-v-84b638d0${_scopeId}><h3 class="text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic leading-none" data-v-84b638d0${_scopeId}> Inisialisasi AMI </h3><p class="text-[10px] font-bold text-rose-500 uppercase tracking-[0.3em] mt-4 italic" data-v-84b638d0${_scopeId}> Buat Penugasan &amp; Snapshot Data Baru </p></div><form class="p-12 space-y-10 bg-white dark:bg-slate-900" data-v-84b638d0${_scopeId}><div class="grid grid-cols-2 gap-8" data-v-84b638d0${_scopeId}><div class="space-y-3" data-v-84b638d0${_scopeId}><label class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1" data-v-84b638d0${_scopeId}>Periode AMI</label><select name="period_id" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-900 dark:text-white shadow-inner focus:ring-2 focus:ring-rose-500/20" data-v-84b638d0${_scopeId}><option value="" disabled data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).period_id) ? ssrLooseContain(unref(form).period_id, "") : ssrLooseEqual(unref(form).period_id, "")) ? " selected" : ""}${_scopeId}>Pilih Periode</option><!--[-->`);
              ssrRenderList(__props.periods, (p) => {
                _push2(`<option${ssrRenderAttr("value", p.id)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).period_id) ? ssrLooseContain(unref(form).period_id, p.id) : ssrLooseEqual(unref(form).period_id, p.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(p.name)}</option>`);
              });
              _push2(`<!--]--></select>`);
              if (unref(form).errors.period_id) {
                _push2(`<p class="text-xs text-rose-500 font-bold ml-1" data-v-84b638d0${_scopeId}>${ssrInterpolate(unref(form).errors.period_id)}</p>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div><div class="space-y-3" data-v-84b638d0${_scopeId}><label class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1" data-v-84b638d0${_scopeId}>Standard Mutu</label><select name="master_standard_id" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-900 dark:text-white shadow-inner focus:ring-2 focus:ring-rose-500/20" data-v-84b638d0${_scopeId}><option value="" disabled data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).master_standard_id) ? ssrLooseContain(unref(form).master_standard_id, "") : ssrLooseEqual(unref(form).master_standard_id, "")) ? " selected" : ""}${_scopeId}>Pilih Instrumen</option><!--[-->`);
              ssrRenderList(__props.standards, (s) => {
                _push2(`<option${ssrRenderAttr("value", s.id)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).master_standard_id) ? ssrLooseContain(unref(form).master_standard_id, s.id) : ssrLooseEqual(unref(form).master_standard_id, s.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(s.name)}</option>`);
              });
              _push2(`<!--]--></select>`);
              if (unref(form).errors.master_standard_id) {
                _push2(`<p class="text-xs text-rose-500 font-bold ml-1" data-v-84b638d0${_scopeId}>${ssrInterpolate(unref(form).errors.master_standard_id)}</p>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div></div><div class="space-y-3" data-v-84b638d0${_scopeId}><label class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1" data-v-84b638d0${_scopeId}>Kategori Unit</label><div class="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-3xl" data-v-84b638d0${_scopeId}><button type="button" class="${ssrRenderClass([unref(form).assignable_type === "prodi" ? "bg-white dark:bg-slate-700 text-rose-600 shadow-sm" : "text-slate-400 dark:text-slate-500", "flex-1 py-4 text-[10px] font-black uppercase rounded-2xl transition-all duration-300"])}" data-v-84b638d0${_scopeId}>Program Studi</button><button type="button" class="${ssrRenderClass([unref(form).assignable_type === "faculty" ? "bg-white dark:bg-slate-700 text-rose-600 shadow-sm" : "text-slate-400 dark:text-slate-500", "flex-1 py-4 text-[10px] font-black uppercase rounded-2xl transition-all duration-300"])}" data-v-84b638d0${_scopeId}>Fakultas</button></div></div><div class="grid grid-cols-2 gap-8" data-v-84b638d0${_scopeId}><div class="space-y-3" data-v-84b638d0${_scopeId}><label class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1" data-v-84b638d0${_scopeId}>Target Unit</label><select name="assignable_id" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-900 dark:text-white shadow-inner focus:ring-2 focus:ring-rose-500/20" data-v-84b638d0${_scopeId}><option value="" disabled data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).assignable_id) ? ssrLooseContain(unref(form).assignable_id, "") : ssrLooseEqual(unref(form).assignable_id, "")) ? " selected" : ""}${_scopeId}>Unit Kerja</option><!--[-->`);
              ssrRenderList(unref(form).assignable_type === "prodi" ? __props.prodis : __props.faculties, (item) => {
                _push2(`<option${ssrRenderAttr("value", item.id)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).assignable_id) ? ssrLooseContain(unref(form).assignable_id, item.id) : ssrLooseEqual(unref(form).assignable_id, item.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(item.name)}</option>`);
              });
              _push2(`<!--]--></select>`);
              if (unref(form).errors.assignable_id) {
                _push2(`<p class="text-xs text-rose-500 font-bold ml-1" data-v-84b638d0${_scopeId}>${ssrInterpolate(unref(form).errors.assignable_id)}</p>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div><div class="space-y-3" data-v-84b638d0${_scopeId}><label class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1" data-v-84b638d0${_scopeId}>Lead Auditor</label><select name="auditor_id" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-900 dark:text-white shadow-inner focus:ring-2 focus:ring-rose-500/20" data-v-84b638d0${_scopeId}><option value="" disabled data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).auditor_id) ? ssrLooseContain(unref(form).auditor_id, "") : ssrLooseEqual(unref(form).auditor_id, "")) ? " selected" : ""}${_scopeId}>Pilih Auditor</option><!--[-->`);
              ssrRenderList(__props.auditors, (a) => {
                _push2(`<option${ssrRenderAttr("value", a.id)} data-v-84b638d0${ssrIncludeBooleanAttr(Array.isArray(unref(form).auditor_id) ? ssrLooseContain(unref(form).auditor_id, a.id) : ssrLooseEqual(unref(form).auditor_id, a.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(a.name)}</option>`);
              });
              _push2(`<!--]--></select>`);
              if (unref(form).errors.auditor_id) {
                _push2(`<p class="text-xs text-rose-500 font-bold ml-1" data-v-84b638d0${_scopeId}>${ssrInterpolate(unref(form).errors.auditor_id)}</p>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div></div><button type="submit"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} class="w-full py-6 bg-slate-900 dark:bg-rose-600 text-white text-xs font-black uppercase tracking-[0.3em] rounded-[2.5rem] shadow-2xl hover:bg-rose-600 dark:hover:bg-rose-500 transition-all active:scale-95 disabled:opacity-50" data-v-84b638d0${_scopeId}>${ssrInterpolate(unref(form).processing ? "Membuat Snapshots..." : "Inisialisasi Siklus AMI Baru")}</button></form></div></div>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
...
const Index = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-84b638d0"]]);
export {
  Index as default
};
