  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.activities, (log) => {
        _push(`<tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"><td class="px-6 py-4 font-bold text-slate-700 dark:text-slate-200"><span class="font-mono text-sm font-black text-rose-500 bg-rose-50 dark:bg-rose-500/10 px-2 py-1 rounded-md border border-rose-100 dark:border-rose-500/20">${ssrInterpolate(__props.activities.from + __props.activities.data.indexOf(log))}</span></td><td class="px-6 py-4 font-bold text-slate-700 dark:text-slate-200">${ssrInterpolate(log.user?.name || "Tidak Diketahui")}</td><td class="px-6 py-4 text-slate-600 dark:text-slate-400">${ssrInterpolate(log.action)}</td><td class="px-6 py-4"><span class="px-2 py-1 rounded text-[10px] font-black uppercase bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300">${ssrInterpolate(log.stage)}</span></td><td class="px-6 py-4 text-slate-400 text-xs">${ssrInterpolate(new Date(log.created_at).toLocaleString("id-ID"))}</td></tr>`);
      });
      _push(`<!--]-->`);
